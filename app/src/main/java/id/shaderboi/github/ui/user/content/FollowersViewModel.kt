...
@HiltViewModel
class FollowersViewModel @Inject constructor(private val userUseCases: UserUseCases) : ViewModel() {
    private val _users = MutableStateFlow<Resource<List<UserBrief>>>(Resource.Loading())
    val users = _users.asStateFlow()

    fun fetchFollowers(username: String) = viewModelScope.launch {
        _users.value = Resource.Loading()
        try {
            _users.value = Resource.Loaded(userUseCases.getUserFollowers(username))
        } catch (ex: Exception) {
